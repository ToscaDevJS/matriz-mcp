...
type ExportWebIn struct {
	Ref       string   `json:"ref" jsonschema:"project-relative path of source image asset"`
	Slot      string   `json:"slot,omitempty" jsonschema:"optional manifest slot ID"`
	Formats   []string `json:"formats,omitempty" jsonschema:"list of formats (e.g. webp, avif); defaults to [avif, webp]"`
	Widths    []int    `json:"widths,omitempty" jsonschema:"custom widths; defaults to [420, 768, 1024, 1440, 1920]"`
	Quality   int      `json:"quality,omitempty" jsonschema:"1..100 quality; default 80"`
	SizesHint string   `json:"sizes_hint,omitempty" jsonschema:"suggested HTML sizes attribute string"`
}

type ExportWebOut struct {
	Variants  []core.ExportVariant          `json:"variants"`
	Srcset    map[core.ImageFormat]string   `json:"srcset"`
	SizesHint string                        `json:"sizes_hint,omitempty"`
}

func handleExportWeb(cfg *config.Config) mcp.ToolHandlerFor[ExportWebIn, ExportWebOut] {
	return func(ctx context.Context, req *mcp.CallToolRequest, in ExportWebIn) (*mcp.CallToolResult, ExportWebOut, error) {
		srcRef := core.AssetRef(in.Ref)
		srcPath, err := core.ResolveRef(cfg.ProjectRoot, srcRef)
		if err != nil {
			return toolError(err, "Make sure the ref path is valid and inside project root."), ExportWebOut{}, nil
		}

		f, err := os.Open(srcPath)
		if err != nil {
			return toolError(fmt.Errorf("source asset %q does not exist: %w", in.Ref, core.ErrAssetNotFound), "Verify the asset exists in the project."), ExportWebOut{}, nil
		}
		defer f.Close()

		srcImg, _, err := image.Decode(f)
		if err != nil {
			return toolError(fmt.Errorf("failed to decode image %s: %w", in.Ref, err), "Ensure valid image format."), ExportWebOut{}, nil
		}

		var formats []core.ImageFormat
		for _, fmtStr := range in.Formats {
			if f, err := core.ParseFormat(fmtStr); err == nil {
		...
		opts := core.ExportOptions{
			...
			Formats:     formats,
			Quality:     in.Quality,
			SizesHint:   in.SizesHint,
		}

		res, err := core.ExportWeb(srcImg, opts)
		if err != nil {
			return toolError(err, "Failed during responsive export."), ExportWebOut{}, nil
		}

		thumb, err := thumbnailContent(srcImg, 512)
		if err != nil {
			return toolError(err, "Failed generating thumbnail preview."), ExportWebOut{}, nil
		}

		out := ExportWebOut{
			Variants:  res.Variants,
			Srcset:    res.Srcset,
			SizesHint: res.SizesHint,
		}
